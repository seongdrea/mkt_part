...
# 사이드바 설정
st.sidebar.title("마케팅파트 RPA")
# 사이드바에 버튼 추가 (if 하고 들여쓰기 하고 사이드바에 버튼 만들면 앞으로 페이지별로 구분할 수 있다)
if st.sidebar.button("네이버쇼핑 검색순위 크롤러"):

    #본격 내용
    st.title("네이버쇼핑 검색순위 크롤러") # 타이틀 설정
    st.write("키워드를 선정하고 버튼을 클릭하면 시작됩니다") # 사용설명
    ...
    # 멀티셀렉트 생성
    keyword_list = st.multiselect("Keyword 추가/수정/삭제는 윤환쓰에게 알려주세요:otter:", options, default=["esim","일본 esim","베트남 esim","태국 esim","대만 esim","필리핀 esim","미국 esim","유럽 esim"])

    # 버튼을 추가.
    if st.button('크롤링 시작하기'):
